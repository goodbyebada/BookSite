...
export default function Book({ bookInfo }: { bookInfo: bookDataProps }) {
  ...
  const [singleBookSize, setSingleBookSize] = useState();

  // const divWidth = {
  //   width: `${parentWidth / 4}px`, // 부모 너비의 1/4로 설정
  //   // 다른 스타일 속성들도 설정 가능
  // };
  ...
  return (
    <div className="singleBook d-flex p-3 flex-column bg-light">
      ...
      <div className="d-flex">
        <span className="me-1">{words[0]}</span>
        <span>{words[1]}</span>
      </div>
    </div>
  );
